...
<body>
      ...
      <div class="container">
        <nav aria-label="breadcrumb">
          <ol class="breadcrumb bg-white px-0 m-0">
            ...
            <li class="breadcrumb-item"><a href="../profile/history.php">ประวัติการสั่งซื้อ</a></li>
            <li class="breadcrumb-item active" aria-current="page">20220726-08040637</li>
          </ol>
        </nav>
      </div>
    </section>
    ...
    <section class="py-5">
      <div class="container">
                ...
                <div class="row">
                  <div class="col-lg-12 col-md-12 pt-3">
                    <div class="d-flex justify-content-between">
                      ...
                      <button type="button" class="btn btn-light btn-sm">เปลี่ยนที่อยู่</button>
                    </div>
                    <table class="table-borderless table">
                      <tr>
        ...
        <div class="row px-2 align-items-center">
          <?php $pay = ($_GET['pay'] == '' ? 'bankTransfer' : $_GET['pay']); ?>
          <div class="col-md my-1 px-2 d-flex flex-wrap align-items-center justify-content-end">
            ช่องทางการชำระเงิน
            <div class="form-check ml-3">
              <input class="form-check-input" type="radio" name="inputpay" id="radioTypeCreditCard" value="creditCard" <?= ($pay == 'creditCard' ? 'checked' : '') ?>>
              <label class="form-check-label ml-1" for="radioTypeCreditCard">บัตรเครดิต / Credit Card</label>
            </div>
            <div class="form-check ml-3">
              <input class="form-check-input" type="radio" name="inputpay" id="radioTypeBankTransfer" value="bankTransfer" <?= ($pay == 'bankTransfer' ? 'checked' : '') ?>>
              <label class="form-check-label ml-1" for="radioTypeBankTransfer">โอนเงินผ่านบัญชีธนาคาร</label>
            </div>
          </div>
          <div class="col-xl-2 col-md-3 col-6 my-1 ml-auto px-2">
            <button type="button" class="btn btn-primary btn-sm w-100 div_bankTransfer" style="display: <?= ($pay == 'creditCard' ? 'block' : 'none') ?>;" data-toggle="modal" data-target="#popup_credit">ชำระเงิน</button>
            <a href="../profile/payment2.php" class="btn btn-primary btn-sm w-100 div_creditCard" style="display: <?= ($pay == 'bankTransfer' ? 'block' : 'none') ?>;">ชำระเงิน</a>
          </div>
        </div>
        <hr>
        <div class="row px-2">
          ...
          <div class="col-xl-2 col-md-3 col-6 my-1 px-2">
            <a href="#" type="button" class="btn btn-light btn-sm w-100" download>ดาวน์โหลดใบเสร็จ</a>
          </div>
          <div class="col-xl-2 col-md-3 col-6 my-1 px-2">
            <button type="button" class="btn btn-soft-danger btn-sm w-100">ยกเลิกคำสั่งซื้อ</button>
          </div>
        </div>
      </div>
    </section>
  ...
  <script>
    $(document).ready(function() {
      $('input:radio[name=inputpay]').click(function() {
        checkTaxType();
      });
    });

    function checkTaxType() {
      var isInputPay = $("input[name=inputpay]:checked").val();
      if (isInputPay === 'bankTransfer') {
        $(".div_creditCard").show();
        $(".div_bankTransfer").hide();
      } else if (isInputPay === 'creditCard') {
        $(".div_bankTransfer").show();
        $(".div_creditCard").hide();
      } else {
        $(".div_creditCard").hide();
        $(".div_bankTransfer").hide();
